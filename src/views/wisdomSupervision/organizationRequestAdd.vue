<template>
  <div class="report-container">
      ...
      <span @click="headerParamClick">报告人基本情况</span>
    </div>
...
export default {
  ...
  data() {
    return {
      ...
      // 追加表头数据
      headerAppend: tableData1[0].headerAppend ? this.recursionList(tableData1[0].headerAppend): [],
      tableData: tableData1[0].data,
      columns: this.recursionList(tableData1[0].columns),
      appendTab: [],
      checked: true,
      // 11111
    }
  ...
  methods: {
    //打印
    PrintRow(index, row){
      this.$print(this.$refs.printCon) // 使用
    },
    headerParamClick(){
    ...
    listClick(index){
      ...
      if(this.menu_active == 0){
        ...
        // 是否为表头表格
        this.headerTab = tableData1[index].headerTab ? tableData1[index].headerTab : 1
        // 表头表格存储数据
        this.headerParam = tableData1[index].headerParam ? tableData1[index].headerParam : {}
        // 追加表头数据
        this.headerAppend = tableData1[index].headerAppend ? this.recursionList(tableData1[index].headerAppend) : []
        // 列数据
        this.columns = this.recursionList(tableData1[index].columns)
        // 表格数据
        ...
        // 追加表头数据
        this.headerAppend = tableData2[index].headerAppend ? this.recursionList(tableData2[index].headerAppend) : []
        this.columns = this.recursionList(tableData2[index].columns)
