<template>
  <div class="organizationRequestDetail">
    <div class='flex flex-align-center h24 w-full border-box' style="color:#121518">
			...
			<span class='pl8 txt-bold font14' style="">详情（事项申报）</span>
    </div>
    <div class="detail_left" v-if='data.reportType ==113'>
      <table  class="detail_tab">
        <tr>
          <td>事项类型：</td>
          <td>{{formData.reportDesc}}</td>
        </tr>
      </table>
    </div>
                        ...
                        <div class="detail_left" v-if='data.reportType ==201'>
                            <table class="detail_tab">
                              ...
                              <tr class='o_tr'>
                                  ...
                                  <td>{{new Date(formData.startTime).toLocaleString('chinese',{hour12: false})}}至{{new Date(formData.applyEnd).toLocaleString('chinese',{hour12: false})}}</td>
                                </tr>
                            </table>
                          </div>                                                                    
          ...
          <el-step title="步骤 2">
            <span slot="title" style="background:none;">
              ...
              <div v-if='data.approvalStatus ==2'>
                  <span class="o_agree">已通过</span>
              </div>   
              <div v-if='data.approvalStatus ==3'>
                  <span class="o_norgee">已驳回</span>
